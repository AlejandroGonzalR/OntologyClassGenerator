...
mxGraphView.prototype.gridImage = (mxClientMin.IS_SVG) ? 'data:image/gif;base64,R0lGODlhCgAKAJEAAAAAAP///8zMzP///yH5BAEAAAMALAAAAAAKAAoAAAIJ1I6py+0Po2wFADs=' :
	IMAGE_PATH + '/grid.gif';
...
Graph = function(container, model, renderHint, stylesheet, themes, standalone)
{
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
	    ...
	    {
	    		return isToggleEvent.apply(this, arguments) || (!mxClientMin.IS_CHROMEOS && mxEvent.isShiftDown(evt));
	    };
	    ...
	    rubberband.isForceRubberbandEvent = function(me)
	    {
		    	return (isForceRubberBandEvent.apply(this, arguments) && !mxEvent.isShiftDown(me.getEvent()) &&
		    		!mxEvent.isControlDown(me.getEvent())) || (mxClientMin.IS_CHROMEOS && mxEvent.isShiftDown(me.getEvent())) ||
		    		(mxUtils.hasScrollbars(this.graph.container) && mxClientMin.IS_FF &&
		    		mxClientMin.IS_WIN && me.getState() == null && mxEvent.isTouchEvent(me.getEvent()));
	    };
 ...
 */
Graph.touchStyle = mxClientMin.IS_TOUCH || (mxClientMin.IS_FF && mxClientMin.IS_WIN) || navigator.maxTouchPoints > 0 ||
 ...
 */
Graph.prototype.defaultScrollbars = !mxClientMin.IS_IOS;
...
(function()
{
	...
	{
		return urlParams['zoom'] != 'nocss' && !mxClientMin.NO_FO && !mxClientMin.IS_EDGE &&
	...
	{
		return this.dialect == mxConstants.DIALECT_SVG && !mxClientMin.NO_FO &&
			(!this.lightbox || !mxClientMin.IS_SF);
	};
...
{
	return mxEvent.isAltDown(evt) || (mxEvent.isMetaDown(evt) && mxClientMin.IS_MAC) ||
...
{
	return mxEvent.isAltDown(evt) || (mxClientMin.IS_CHROMEOS && mxEvent.isShiftDown(evt));
};
 ...
 * Up arrow.
 */
HoverIcons.prototype.triangleUp = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/triangle-up.png', 26, 14) :
 ...
 * Right arrow.
 */
HoverIcons.prototype.triangleRight = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/triangle-right.png', 14, 26) :
 ...
 * Down arrow.
 */
HoverIcons.prototype.triangleDown = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/triangle-down.png', 26, 14) :
 ...
 * Left arrow.
 */
HoverIcons.prototype.triangleLeft = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/triangle-left.png', 14, 26) :
 ...
 * Round target.
 */
HoverIcons.prototype.roundDrop = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/round-drop.png', 26, 26) :
...
HoverIcons.prototype.createArrow = function(img, tooltip)
{
	...
	if (mxClientMin.IS_IE && !mxClientMin.IS_SVG)
	{
		// Workaround for PNG images in IE6
		if (mxClientMin.IS_IE6 && document.compatMode != 'CSS1Compat')
		{
			arrow = document.createElement(mxClientMin.VML_PREFIX + ':image');
		...
		arrow.style.display = (mxClientMin.IS_QUIRKS) ? 'inline' : 'inline-block';
	}
	else
	{
...
if (typeof mxVertexHandler != 'undefined')
{
	(function()
	{
			...
			if (this.isEnabled())
			{
				...
				if (evt != null && !this.model.isVertex(cell))
				{
					...
					if ((this.firstClickState == state && this.firstClickSource == src) &&
						...
						(state != null || (mxClientMin.IS_VML && src == this.view.getCanvas()) ||
						(mxClientMin.IS_SVG && src == this.view.getCanvas().ownerSVGElement)))
					{
		...
		{
			return (mxClientMin.IS_MAC && mxEvent.isMetaDown(evt)) || mxEvent.isControlDown(evt);
		};
		...
		mxCellEditor.prototype.startEditing = function(cell, trigger)
		{
				...
				this.graph.getModel().isEdge(cell))
			{
				...
				if (mxClientMin.IS_QUIRKS)
				{
					this.textarea.style.border = 'gray dotted 1px';
				}
				// IE>8 and FF on Windows uses outline default of none
				else if (mxClientMin.IS_IE || mxClientMin.IS_IE11 || (mxClientMin.IS_FF && mxClientMin.IS_WIN))
				{
		...
		mxCellEditor.prototype.installListeners = function(elt)
		{
			...
			if (!mxClientMin.IS_QUIRKS && document.documentMode !== 7 && document.documentMode !== 8)
			{
		...
		mxCellEditor.prototype.toggleViewMode = function()
		{
			...
			if (state != null)
			{
				...
				if (!this.codeViewMode)
				{
					...
					if (!mxClientMin.IS_QUIRKS && document.documentMode != 8)
					{
		...
		HoverIcons.prototype.mainHandle = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-main.png', 17, 17) :
			Graph.createSvgImage(18, 18, '<circle cx="9" cy="9" r="5" stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '" stroke-width="1"/>');
		HoverIcons.prototype.secondaryHandle = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-secondary.png', 17, 17) :
			Graph.createSvgImage(16, 16, '<path d="m 8 3 L 13 8 L 8 13 L 3 8 z" stroke="#fff" fill="#fca000"/>');
		HoverIcons.prototype.fixedHandle = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-fixed.png', 17, 17) :
		...
		HoverIcons.prototype.terminalHandle = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-terminal.png', 17, 17) :
		...
		HoverIcons.prototype.rotationHandle = (!mxClientMin.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-rotate.png', 16, 16) :
		...
		if (Graph.touchStyle)
		{
			...
			if (mxClientMin.IS_TOUCH || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0)
			{
