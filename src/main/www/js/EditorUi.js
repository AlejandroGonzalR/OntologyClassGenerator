...
EditorUi = function(editor, container, lightbox)
{
	...
	if (!mxClientMin.IS_SVG && mxConnectionHandler.prototype.connectImage != null)
	{
	...
	if (!graph.standalone)
	{
		...
		if (!this.editor.chromeless || this.editor.editable)
		{
			...
			if (mxClientMin.IS_IE && (typeof(document.documentMode) === 'undefined' || document.documentMode < 9))
			{
	    ...
	    if (mxClientMin.IS_SVG && graph.view.getDrawPane() != null)
	    {
		...
		graph.cellEditor.isStopEditingEvent = function(evt)
		{
			return cellEditorIsStopEditingEvent.apply(this, arguments) ||
				(evt.keyCode == 13 && ((!mxClientMin.IS_SF && mxEvent.isControlDown(evt)) ||
				(mxClientMin.IS_MAC && mxEvent.isMetaDown(evt)) ||
				(mxClientMin.IS_SF && mxEvent.isShiftDown(evt))));
		};
		...
		if (mxClientMin.IS_IOS && !window.navigator.standalone)
		{
 ...
 */
EditorUi.prototype.splitSize = (mxClientMin.IS_TOUCH || mxClientMin.IS_POINTER) ? 12 : 8;
	...
	if (this.editor.isChromelessView())
	{
        resize = mxUtils.bind(this, function(autoscale, maxScale, cx, cy)
        {
            if (graph.container != null && !graph.isViewer())
            {
                ...
                var sb = (mxClientMin.IS_QUIRKS || document.documentMode >= 8) ? 20 : 14;
		...
		if (urlParams['toolbar'] != '0')
		{
			...
			mxEvent.addListener(graph.container, (mxClientMin.IS_POINTER) ? 'pointermove' : 'mousemove', mxUtils.bind(this, function(evt)
			{
			...
			mxEvent.addListener(this.chromelessToolbar, (mxClientMin.IS_POINTER) ? 'pointermove' : 'mousemove', function(evt)
			{
				mxEvent.consume(evt);
			});
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
	...
	var quirks = mxClientMin.IS_IE && (document.documentMode == null || document.documentMode == 5);
	...
	if (tmp > 0 && !mxClientMin.IS_QUIRKS)
	{
		tmp += 1;
	}
...
EditorUi.prototype.createKeyHandler = function(editor)
{
	...
	keyHandler.isEventIgnored = function(evt)
	{
		...
		return (!this.isControlDown(evt) || mxEvent.isShiftDown(evt) || (evt.keyCode != 90 && evt.keyCode != 89 &&
			...
			!this.isControlDown(evt) || (this.graph.cellEditor.isContentEditing() && !mxClientMin.IS_FF && !mxClientMin.IS_SF)) &&
	...
	{
		return mxEvent.isControlDown(evt) || (mxClientMin.IS_MAC && evt.metaKey);
	};
