...
var OpenDialog = function()
{
	...
	var dx = (mxClientMin.IS_VML && (document.documentMode == null || document.documentMode < 8)) ? 20 : 0;
...
var AboutDialog = function(editorUi)
{
	...
	div.appendChild(img);
	mxUtils.br(div);
	mxUtils.write(div, 'Powered by mxGraph ' + mxClientMin.VERSION);
	mxUtils.br(div);
...
var FilenameDialog = function(editorUi, filename, buttonText, fn, label, validateFn, content, helpLink, closeOnBtn, cancelFn, hints, w)
{
	...
	this.init = function()
	{
		...
		if (mxClientMin.IS_GC || mxClientMin.IS_FF || document.documentMode >= 5 || mxClientMin.IS_QUIRKS)
		{
			nameInput.select();
		}
		else
		...
		if (Graph.fileSupport)
		{
			...
			if (dlg != null)
			{
				...
				mxEvent.addListener(dlg, 'dragover', mxUtils.bind(this, function(evt)
				{
					...
					if (dropElt == null && (!mxClientMin.IS_IE || document.documentMode > 10))
					{
						dropElt = nameInput;
...
FilenameDialog.createTypeHint = function(ui, nameInput, hints)
{
	...
	var nameChanged = function()
	{
		...
		for (var i = 0; i < hints.length; i++)
		{
			...
			{
				hint.setAttribute('src',  mxClientMin.imageBasePath + '/warning.png');
...
var ExportDialog = function(editorUi)
{
	...
	var table = document.createElement('table');
	var tbody = document.createElement('tbody');
	table.setAttribute('cellpadding', (mxClientMin.IS_SF) ? '0' : '2');
...
var EditDataDialog = function(ui, cell)
{
	...
	var addRemoveButton = function(text, name)
	{
		...
		img.style.marginBottom = (mxClientMin.IS_IE11) ? '-1px' : '5px';
	...
	nameInput.setAttribute('size', (mxClientMin.IS_IE || mxClientMin.IS_IE11) ? '36' : '40');
	...
	if (ui.editor.graph.getModel().isVertex(cell) || ui.editor.graph.getModel().isEdge(cell))
	{
		...
		if (EditDataDialog.placeholderHelpLink != null)
		{
			...
			icon.style.marginTop = (mxClientMin.IS_IE11) ? '0px' : '-4px';
...
var LinkDialog = function(editorUi, initialValue, btnLabel, fn)
{
	...
	cross.style.display = (mxClientMin.IS_VML) ? 'inline' : 'inline-block';
	cross.style.top = ((mxClientMin.IS_VML) ? 0 : 3) + 'px';
	...
	function refresh()
	{
		...
		function addLayer(index, label, child, defaultParent)
		{
			...
			if (graph.isEnabled())
			{
				...
				if (mxClientMin.IS_TOUCH || mxClientMin.IS_POINTER || mxClientMin.IS_VML ||
					(mxClientMin.IS_IE && document.documentMode < 10))
				{
				...
				if (mxClientMin.IS_SVG && (!mxClientMin.IS_IE || document.documentMode >= 10))
				{
