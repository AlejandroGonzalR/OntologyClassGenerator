...
Format.prototype.refresh = function()
{
	...
	label.style.display = (mxClientMin.IS_QUIRKS) ? 'inline' : 'inline-block';
	label.style.height = (mxClientMin.IS_QUIRKS) ? '34px' : '25px';
	label.style.overflow = 'hidden';
	label.style.width = '100%';
	this.container.appendChild(div);
	
	// Prevents text selection
    mxEvent.addListener(label, (mxClientMin.IS_POINTER) ? 'pointerdown' : 'mousedown',
	...
	else
	{
		...
		var addClickHandler = mxUtils.bind(this, function(elt, panel, index)
		{
			...
			// Prevents text selection
		    mxEvent.addListener(elt, (mxClientMin.IS_POINTER) ? 'pointerdown' : 'mousedown',
	        	mxUtils.bind(this, function(evt)
	    	{
				evt.preventDefault();
			}));
...
BaseFormatPanel.prototype.createStepper = function(input, update, step, height, disableFocus, defaultValue, isFloat)
{
	...
	else if (mxClientMin.IS_MT || document.documentMode >= 8)
	{
		height = height + 1;
	} 
...
BaseFormatPanel.prototype.createOption = function(label, isCheckedFn, setCheckedFn, listener)
{
	...
	div.style.overflow = 'hidden';
	div.style.width = '200px';
	div.style.height = (mxClientMin.IS_QUIRKS) ? '27px' : '18px';
...
BaseFormatPanel.prototype.createColorOption = function(label, getColorFn, setColorFn, defaultColor, listener, callbackFn, hideCheckbox)
{
	...
	var apply = function(color, disableUpdate, forceUpdate)
	{
		if (!applying)
		{
			...
			btn.innerHTML = '<div style="width:' + ((mxClientMin.IS_QUIRKS) ? '30' : '36') +
	...
	btn.style.right = (mxClientMin.IS_QUIRKS) ? '0px' : '20px';
...
BaseFormatPanel.prototype.addArrow = function(elt, height)
{
	...
	arrow.innerHTML = '<img border="0" src="' + ((mxClientMin.IS_SVG) ? 'data:image/png;base64,' :
	...
	if (symbol != null)
	{
		...
		symbol.style.display = (mxClientMin.IS_QUIRKS) ? 'inline' : 'inline-block';
...
TextFormatPanel.prototype.addFont = function(container)
{
	...
	input.style.height = (mxClientMin.IS_QUIRKS) ? '21px' : '17px';
	...
	function(fontSize)
	{
		...
		if (window.getSelection && !mxClientMin.IS_IE && !mxClientMin.IS_IE11)
		{
	...
	{
		if (mxClientMin.IS_IE && (mxClientMin.IS_QUIRKS || document.documentMode < 10))
		{
	...
	if (graph.cellEditor.isContentEditing())
	{
			...
			if (!updating)
			{
				...
				window.setTimeout(function()
				{
					...
					if (node != null)
					{
						...
						// IE does not support containsNode
						if (elts.length > 0 && window.getSelection && !mxClientMin.IS_IE && !mxClientMin.IS_IE11)
						{
							var selection = window.getSelection();
		...
		if (mxClientMin.IS_FF || mxClientMin.IS_EDGE || mxClientMin.IS_IE || mxClientMin.IS_IE11)
		{
...
StyleFormatPanel.prototype.addFill = function(container)
{
	...
	gradientSelect.style.right = (mxClientMin.IS_QUIRKS) ? '52px' : '72px';
...
DiagramFormatPanel.prototype.addView = function(div)
{
	...
	if (graph.isEnabled())
	{
		...
		if (this.showBackgroundImageOption)
		{
			...
			btn.className = 'geColorBtn';
			btn.style.marginTop = '-4px';
			btn.style.paddingBottom = (document.documentMode == 11 || mxClientMin.IS_MT) ? '0px' : '2px';
			btn.style.height = '22px';
			btn.style.right = (mxClientMin.IS_QUIRKS) ? '52px' : '72px';
